...
def compute_grad1(model: nn.Module, loss_type: str = 'mean') -> None:
    ...
    for layer in model.modules():
        ...
        if layer_type == 'Linear':
            ...
            grad1 = torch.nn.functional.conv_transpose2d(input=A, weight=B)
            print(f'A1: {A.shape}, B1: {B.shape}, grad1: {grad1.shape}')
...
def manual_unfold(input_data, kernel_size, stride=1, padding=0, dilation=1):
    # 转换为PyTorch张量
    ...
    unfolded_data = torch.zeros(batch_size, kernel_height * kernel_width, output_height * output_width).to(
        torch.device('mps')
    )
    ...
    for i in range(output_height):
        for j in range(output_width):
            ...
            h_end = h_start + (kernel_height - 1) * dilation + 1
            w_end = w_start + (kernel_width - 1) * dilation + 1

            patch = input_data[:, :, h_start:h_end:dilation, w_start:w_end:dilation].reshape(batch_size, -1)
            unfolded_data[:, :, i * output_width + j] = patch
